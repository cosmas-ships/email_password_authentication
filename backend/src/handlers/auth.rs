...
pub async fn logout(
    ...
    let claims = state.jwt_service.verify_access_token(&payload.access_token)?;
    let user_id = Uuid::parse_str(&claims.sub).map_err(|_| AppError::InvalidToken)?;

    // Blacklist access token in Redis so it can’t be reused
    state
        .token_service
    ...
    let sessions_revoked = if payload.logout_all {
        ...
        // Revoke *only* the one used in this session
        state.token_service.revoke_token(&payload.refresh_token).await?;
        1
    };
